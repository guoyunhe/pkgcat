...
/**
 * Writes the repositories of the catalog that no distribution ships, which are read from
 * `database/data`. A repository is stored once per base URL and linked to every release its
 * `distros` selectors name, so a repository a whole family of releases installs from is one entry
 * with several entries behind it.
 */
export default class RepoSeeder extends BaseSeeder {
  async run() {
    for (const { distros, ...attributes } of communityRepos) {
