 ...
 * and the `parent` category code, which is `null` for a top level category. The display names of
 * the categories are translations of the interface and live in
 * `public/locales/<language>/categories.json`, which `node ace category:names` writes out of the
 * menu definitions the desktop ships.
 *
...
/** One entry of the registry: the AppStream code and the parent category code. */
type CategorySeed = {
  code: string
  parent: string | null
}
