 ...
 * desktop, whose labels KDE translates into ninety-odd languages, so the file needs no hand-written
 * translations.
 */
export default class CategoryNames extends BaseCommand {
  static commandName = 'category:names'
  static description = 'Write the display names of the categories from the KDE menu definitions'
  ...
  @flags.string({
    ...
    default: '/etc/xdg/menus/kf5-applications.menu',
  })
  declare menu: string
  ...
  async run() {
    ...
    for (const [index, menu] of menus.entries()) {
      ...
      for (const child of this.children(menus, index)) {
        const categories = new Set(child.categories)
        shared = shared === null ? categories : shared.intersection(categories)
      }
    ...
    for (const [code, [slug, translations]] of [...assigned].sort(([left], [right]) =>
      ...
      this.logger.info(
        `${chalk.green(code.padEnd(16))} ${slug.padEnd(36)}` +
          chalk.dim(` ${output[code].en ?? code} (${Object.keys(translations).length} locales)`),
      )
    }

    // One file per language, which i18next reads as the `categories` namespace of that language
    const languages = new Map<string, Record<string, string>>()
    for (const [code, perLanguage] of Object.entries(output)) {
      for (const [language, name] of Object.entries(perLanguage)) {
        languages.set(language, { ...languages.get(language), [code]: name })
      }
    }

    for (const [language, translated] of [...languages].sort(([left], [right]) =>
      left.localeCompare(right),
    )) {
    ...
    this.logger.info(
      `${Object.keys(output).length} categories named in ${languages.size} languages, written to ${chalk.cyan(this.out)}`,
    )
  }
  ...
  private async readNames(): Promise<Map<string, DirectoryNames>> {
    ...
    for (const file of await readdir(this.directories)) {
      if (!file.startsWith('kf5-') || !file.endsWith('.directory')) continue
...
function categoriesOf(include: ParsedInclude | undefined): string[] {
  ...
  const categories = group.length > 0 ? group : (include.Category ?? [])
  return categories.filter(
    (code) => !code.startsWith(reservedPrefix) && !reservedCategories.has(code),
  )
}
